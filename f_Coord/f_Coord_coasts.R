...
Coord_coasts <- function(points, names_coord, region, layer_coastlines, elements_file) {
  print("a")
  ...
  layer_name <- paste(region, "_uniques")

  elements <- st_read(elements_file, layer = layer_name)
  elements <- st_transform(france, 4326)
