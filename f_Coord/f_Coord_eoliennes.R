...
Coord_eol=function(points,names_coord,bs,bm,bl,layer1, VCFolder, NameGrid)
{
  ...
  if(length(BufferS$pt_count)>0)
  {
    ...
    OccSL_Re=merge(OccSL,PC_50,by.x="FID",by.y="Group.1",all.x=T)
  ...
  if(length(BufferM$pt_count)>0)
  {
    ...
    OccSL_Re=merge(OccSL_Re,PC_50,by.x="FID",by.y="Group.1",all.x=T)
  ...
  OccSL_ARajouter=subset(OccSL_Re_WGS84,select=grepl("Sp",names(OccSL_Re_WGS84)))
  
  Reseau=data.frame(cbind(st_coordinates(OccSL_WGS84), as.data.frame(OccSL_ARajouter)))
