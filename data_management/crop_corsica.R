...
# Load SysGrid
sysgrid_path <- file.path(data_path, "observations", "SysGrid_500m_de_cote_france_met.csv")
sysgrid = read_delim(sysgrid_path)
...
data_pred_sf = st_as_sf(data_pred, # convert to sf
    coords = c("X", "Y"), crs=4326, remove=FALSE)
sysgrid_sf = st_as_sf(sysgrid, # convert to sf
    coords = c("X", "Y"), crs=4326, remove=FALSE)
