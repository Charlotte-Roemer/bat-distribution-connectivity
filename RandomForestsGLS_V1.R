...
args[9] <- TRUE #if date (=day-of-year) is provided or not
#args[10]="abondance"
...
if (args[3] != "NA") {# if species list is provided
  ...
  fwrite(SpeciesList,
         paste0("SpeciesList_", Group, substr(Sys.time(), 1, 10), ".csv")
         )
}
...
for (i in 1:length(ListSp)) {
  ...
  if (W0) { # if bat activity table contains 0
    ...
    DataSpSL_w0_2 <- subset(DataSpSL_w0_2,
                            DataSpSL_w0_2$longitude < 10 & DataSpSL_w0_2$longitude > -6 &
